...
def funcForButton():
    ...
    if expression:
        ...
        if not valid(expression):
            ...
            printTheHead(variables, replace(expression, reverseMap))
            if ReversePoland == variables[0]:  # 只有一个字母
                printOneLine(["0"], "0")
                printOneLine(["1"], "1")
                return
            for each in sequenceGenerator(len(variables)):
                ...
                printOneLine(each, str(int(statement.pop())))
    else:
        setTextOut("请输入指令!")
...
textForExplanation.insert(1.0, "你好，该程序用于将中缀表达式转化为真值表！\n"
